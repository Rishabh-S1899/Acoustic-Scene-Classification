...
#                           num_workers=1, shuffle=None, dataset=CMD("/basedataset.get_training_set"),
#                           )

get_train_loader = ex.datasets.training.iter(DataLoader, static_args=dict(worker_init_fn=worker_init_fn), train=True, batch_size=12,
                          num_workers=0, shuffle=True, dataset=CMD("/basedataset.get_training_set"),
                          )

get_validate_loader = ex.datasets.test.iter(DataLoader, static_args=dict(worker_init_fn=worker_init_fn),
                                            validate=True, batch_size=20, num_workers=0,
...
def default_conf():
    ...
    models = {
        "net": DynamicIngredient("models.passt.model_ing", n_classes=10, s_patchout_t=10, s_patchout_f=3),
    ...
    basedataset = DynamicIngredient("Dcase_2020.dataset.dataset")
    trainer = dict(max_epochs=10, gpus=1, weights_summary='full', benchmark=True, num_sanity_val_steps=0,
                   reload_dataloaders_every_epoch=True)
    wandb = dict(project="passt_dcase2020", log_model=True)
    lr = 0.00001
    use_mixup = True
    mixup_alpha = 0.3
