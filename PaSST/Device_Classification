...
devices_in_df1 = data['device'].tolist()

# Filter the second dataframe to keep only rows where the device is present in the first dataframe
test_filtered = test_data[test_data['device'].isin(devices_in_df1)].reset_index()
y_test_one_hot_encoded=one_hot(test_filtered[run_mode],run_mode)

class_names = list(np.unique(test_data[run_mode].tolist()))

criterion = nn.CrossEntropyLoss(weight=torch.from_numpy(weights_array).to(device))
