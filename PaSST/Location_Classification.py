...
# Check if CUDA device is available or not
...
for i in range(1, number_of_transformer_blocks + 1):
    final_X_train = train_ready(data, f'embedding{i}')

    # Split the data into train and validation sets
    X_train, X_val, y_train, y_val = train_test_split(final_X_train, y_one_hot_encoded, test_size=0.2,
                                                      random_state=42, stratify=y_one_hot_encoded)
    ...
    best_loss = np.inf  # Initialize with negative infinity
    best_weights = None
    train_loss_hist = []
    train_acc_hist = []
    test_loss_hist = []
    ...
    for epoch in range(n_epochs):
        ...
        # Update the best weights if the current loss is lower
        if ce < best_loss:
            best_loss = ce
...
for i, (train_loss_hist, test_loss_hist, train_acc_hist, test_acc_hist, run_mode) in enumerate(
        zip(train_loss_tracker, test_loss_tracker, train_acc_tracker, test_acc_tracker, run_mode)):
...
num_matrices = len(y_pred_all_block_list)  # Number of confusion matrices
fig, axes = plt.subplots(1, num_matrices, figsize=(15, 5))

for i, y_pred_matrix in enumerate(y_pred_all_block_list):
    ax = axes[i]
