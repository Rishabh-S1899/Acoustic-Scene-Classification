...
class AudioSetDataset(TorchDataset):
    def __init__(self, csv_file,  audiopath, fold, train=False, sample_rate=32000, classes_num=10,
        ...
        if augment:
            print(f"Will agument data from {csv_file}")
...
@dataset.command
def get_base_training_set(meta_train_csv,audio_train_path,fold=0):
    print(f"this is the fold value being passed : {fold}")
    ds = AudioSetDataset(meta_train_csv, audio_train_path, fold,  train=True, augment=True,classes_num=10)
    return ds
...
@dataset.command
def get_base_test_set(meta_test_csv, audio_test_path, fold=1):
    ds = AudioSetDataset(meta_test_csv, audio_test_path, fold, train=False, classes_num=10)
    return ds
...
if __name__ == "__main__":
    ...
    def default_command():
        ...
        ds = get_test_set()
        print("this is ds after test call \n",ds[0])
        ds = get_training_set()
        # print("this is ds after training_set call \n",ds[0])
